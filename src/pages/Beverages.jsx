...
// Define subcategory IDs for Beverages (based on context)
    ...
    >
      ...
      <div className="relative z-10 flex flex-col items-center justify-center h-full px-6 md:px-24 text-white text-center">
        ...
        <div className="mt-8 flex space-x-4">
          <a
            href="https://wa.me/+YOUR_PHONE_NUMBER?text=Hi%20Maison%20Ember,%20I'd%20like%20to%20order%20a%20beverage!"
            target="_blank"
  ...
  return (
    <div className="bg-white rounded-lg shadow-sm overflow-hidden border border-gray-100">
      ...
      <div className="p-6">
        ...
        <p className="mt-4 text-xs text-gray-500 font-sans">{item.barista || "N/A"}</p>
        <a
  ...
  return (
    <div className="bg-[#fcf8f3] py-12 px-4 sm:px-6 lg:px-8">
      <div className="text-center mb-10">
        <h2 className="text-3xl md:text-4xl font-bold text-gray-800 font-serif">Coffees & Teas</h2>
        <p className="mt-2 text-gray-600 font-sans">Premium blends crafted for a perfect sip</p>
      </div>
      {items.length === 0 ? (
...
const JuicesAndSmoothies = ({ items }) => {
  return (
    <div className="bg-[#fcf8f3] py-12 px-4 sm:px-6 lg:px-8">
      <div className="text-center mb-10">
        <h2 className="text-3xl md:text-4xl font-bold text-gray-800 font-serif">Juices & Smoothies</h2>
        <p className="mt-2 text-gray-600 font-sans">Freshly blended for a vibrant, healthy boost</p>
      </div>
      {items.length === 0 ? (
        <p className="text-center text-gray-600 font-sans">No items available in this category</p>
      ) : (
...
const SodasAndSpecialtyDrinks = ({ items }) => {
  return (
    <div className="bg-[#fcf8f3] py-12 px-4 sm:px-6 lg:px-8">
      <div className="text-center mb-10">
        <h2 className="text-3xl md:text-4xl font-bold text-gray-800 font-serif">Sodas & Specialty Drinks</h2>
        <p className="mt-2 text-gray-600 font-sans">Unique flavors for a refreshing experience</p>
      </div>
      {items.length === 0 ? (
        <p className="text-center text-gray-600 font-sans">No items available in this category</p>
      ) : (
  ...
  return (
    <div className="bg-[#fcf8f3] py-12 px-4 sm:px-6 lg:px-8">
      <div className="max-w-7xl mx-auto text-center">
        <h2 className="text-3xl md:text-4xl font-bold text-gray-800 font-serif">Why Choose Our Beverages?</h2>
        <p className="mt-2 text-gray-600 font-sans">Experience the refreshment of artisanal drinks with every sip</p>
  ...
  return (
    <div className="bg-[#fcf8f3] py-12 px-4 sm:px-6 lg:px-8">
      <div className="max-w-3xl mx-auto text-center p-8 rounded-lg shadow-sm bg-white border border-gray-100">
        <h2 className="text-3xl font-bold text-gray-800 font-serif">Craving Refreshment?</h2>
...
const Beverages = () => {
  ...
  const fetchMenuItems = async () => {
    ...
    setError("");
    try {
      const res = await fetch("http://localhost/food_and_restaurant/get_menu_items.php?category_id=4", {
        method: "GET",
      ...
      if (data.success && Array.isArray(data.data)) {
        setCoffeeTeaItems(data.data.filter(item => item.sub_category_id === SUBCATEGORY_IDS.COFFEES_TEAS || item.sub_category_id === "10"));
        setJuiceSmoothieItems(data.data.filter(item => item.sub_category_id === SUBCATEGORY_IDS.JUICES_SMOOTHIES || item.sub_category_id === "11"));
        setSodaSpecialtyItems(data.data.filter(item => item.sub_category_id === SUBCATEGORY_IDS.SODAS_SPECIALTY || item.sub_category_id === "12"));
      } else {
    ...
    <div className="bg-[#fcf8f3]">
      ...
      {isLoading && (
        <div className="text-center py-12">
          <p className="text-gray-600 font-sans">Loading beverages...</p>
        </div>
      )}
      <FastFoodHero />
